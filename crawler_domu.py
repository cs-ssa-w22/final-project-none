...
def listing_page_crawler(url, headers):
    ...
    for line in soup.find_all("div")[0].find_all("div")[3:]:
        ...
        if a_html != [] and loc != None and price != None:
            properties_info_lst.append(("https://www.domu.com" + a_html[2].get("href"),
                                        loc,
                                        price))
...
def property_page_crawler(url):
    ...
    try:
        # bedroom, bathroom, area range (for apartments)
        bed_num_lst = []
        bath_num_lst = []
    ...
    return property_name, address, bed, ba, area_range, amenities
...
if __name__ == "__main__":
    ...
    with open (output_filename, 'w', newline = '') as csvfile:
        ...
        # Write data
        for prop_tuple in all_properties_links:

            url = prop_tuple[0]
            lat = float(prop_tuple[1].split(',')[0])
            long = float(prop_tuple[1].split(',')[1])
            price_range = prop_tuple[2]

            try:
                name, add, bed, ba, area, amenities = property_page_crawler(url)
            except:
                continue

            row = [name, add, lat, long, price_range, bed, ba, area, amenities]
            writer.writerow(row)
